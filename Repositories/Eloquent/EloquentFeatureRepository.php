...
class EloquentFeatureRepository extends EloquentBaseRepository implements FeatureRepository
{
    public function whereProduct($product_id, $perPage = 15)
    {
        ...
        if (method_exists($this->model, 'translations')) {
            return $this->model->with('translations')->where('product_id',$product_id)->orderBy('id', 'ASC')->paginate($perPage);
        }

        return $this->model->where('product_id',$product_id)->orderBy('id', 'ASC')->paginate($perPage);
    }//whereProduct()

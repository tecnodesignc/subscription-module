...
@section('content-header')
    <h1>
        {{ trans('subscription::features.title.create feature') }}
    </h1>
    <ol class="breadcrumb">
        ...
        <li class="active">{{ trans('subscription::features.title.create feature') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.subscription.feature.store',$product_id], 'method' => 'post']) !!}
                                ...
                                <div class="tab-content">
                                    ...
                                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                        ...
                                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}"
                                             id="tab_{{ $i }}">
                                            @include('subscription::admin.features.partials.create-fields', ['lang' => $locale])
                                        </div>
                                    @endforeach
                                </div>
                        ...
                        <div class="box-body">
                            ...
                            <div class='form-group{{ $errors->has("type") ? ' has-error' : '' }}'>
                                ...
                                <select class="form-control" name="type" id="type">
                                    ...
                                    <option value="0">{{ trans('subscription::types.quantity')}}</option>
                                    <option value="1">{{trans('subscription::types.text')}}</option>
                                    <option value="2">{{trans('subscription::types.boolean')}}</option>
                                </select>
                                {!! $errors->first("type", '<span class="help-block">:message</span>') !!}
                            </div>
                            <div class='form-group{{ $errors->has("unit") ? ' has-error' : '' }}'>
                                {!! Form::label("unit", trans('subscription::features.form.unit')) !!}
                                {!! Form::text("unit", old("unit"), ['class' => 'form-control', 'placeholder' => trans('subscription::features.form.unit')]) !!}

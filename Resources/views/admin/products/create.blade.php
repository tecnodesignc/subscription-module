...
@section('content-header')
    <h1>
        {{ trans('subscription::products.title.create product') }}
    </h1>
    <ol class="breadcrumb">
        ...
        <li class="active">{{ trans('subscription::products.title.create product') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.subscription.product.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-8">
            <div class="row">
                <div class="col-xs-12">
                                ...
                                <div class="tab-content">
                                    ...
                                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                                        ...
                                        <div class="tab-pane {{ App::getLocale() == $locale ? 'active' : '' }}"
                                             id="tab_{{ $i }}">
                                            @include('subscription::admin.products.partials.create-fields', ['lang' => $locale])
                                            <div class='form-group{{ $errors->has("require_shipping_address") ? ' has-error' : '' }}'>
                                                <div class="checkbox">
                                                    <label>
                                                        <input type="checkbox" value="1" class="flat-blue jsInherit"
                                                               name="require_shipping_address"  {{old("require_shipping_address")?' checked="checked"':''}}>
                        ...
                        <div class="box-body">
                                    ...
                                    <div class="row">
                                        <div class="col-xs-12">
                                            <div class='form-group{{ $errors->has("system_name") ? ' has-error' : '' }}'>
                                                ...
                                                {!! Form::text("system_name", old("system_name"), ['class' => 'form-control', 'placeholder' => trans('subscription::products.form.system_name')]) !!}
                            ...
                            <div class="row">
                                <div class="col-xs-12 text-right">
                                    <button type="submit"
                                            class="btn btn-primary btn-flat">{{ trans('subscription::products.button.create product') }}</button>
                                </div>
                            </div>
                                ...
                                <div class="tab-content">
                                    <select name="user_id" id="user" class="form-control">
                                        @foreach ($users as $user)
                                            <option
                                                value="{{$user->id }}" {{$user->id == $currentUser->id ? 'selected' : ''}}>{{$user->present()->fullname()}}
                                                - ({{$user->email}})
                                            </option>
                                        @endforeach
                                    </select>
